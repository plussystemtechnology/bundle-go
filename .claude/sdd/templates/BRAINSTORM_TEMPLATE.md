...
|--------|-------------|-------------|
| Go Version | {e.g., go1.23} | {Affects language features available} |
| Likely Layer | {domain \| port \| app \| adapter \| bootstrap \| cmd} | {Where code should live in Clean Arch} |
| Relevant KB Domains | {domain-1, domain-2, etc.} | {Patterns to consult from .claude/kb/} |
| IaC / Deployment | {K8s / Docker / local / N/A} | {Infrastructure approach} |
...
> Include this section when the feature involves data pipelines, Kafka consumers, or data infrastructure.

### Source Systems
| Source | Type | Volume Estimate | Current Freshness |
|--------|------|-----------------|-------------------|
| {Source 1} | {Postgres / Kafka / Redis / HTTP API} | {~rows/day or msg/s} | {Real-time / Periodic / Unknown} |

### Data Flow Sketch
```text
[Source A] → [Consumer/Handler] → [Service] → [Repository] → [Postgres/Redis]
```
...
| 2 | What freshness SLA is needed? | {Answer} | {Sync vs async, Kafka vs REST} |
