...
```bash
/iterate DEFINE_USER_AUTH.md "Add support for OAuth2 PKCE flow"
/iterate DESIGN_ORDER_SERVICE.md "Switch from in-memory cache to Redis"
/iterate .claude/sdd/features/DEFINE_RATE_LIMITER.md "Change from token bucket to sliding window"
```
...
|-------------|---------|--------|
| **Additive** | "Also support gRPC" | Low - adds to existing |
| **Modifying** | "Change from JWT to session-based" | Medium - updates existing |
| **Removing** | "Remove Kafka integration" | Medium - simplifies |
| **Architectural** | "Switch from REST to gRPC entirely" | High - may require redesign |
...
| Source | Cascades To |
|--------|-------------|
| DEFINE change | May need DESIGN update (new layers, agents) |
| DESIGN change | May need code update (new/changed files) |
...
| Scope expansion | New Clean Architecture layers |
...
|---------------|-------------|
| New file in manifest | Create Go file |
...
2. **Be Specific** - "Add gRPC support" is better than "make it better"
